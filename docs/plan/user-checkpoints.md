...
- The Firebase **web config** (`apiKey`, `projectId`, …) is _not_ secret, but still goes only in the git-ignored `web/.env.production.local`.
...
2. **Authentication** → _Get started_ → _Sign-in method_ → **Google** → Enable → choose a support email → Save.
3. **Firestore Database** → _Create database_:
...
4. **Project settings** (gear icon) → _General_ → _Your apps_ → click the **Web** icon `</>`:
...
2. Firebase console → **Project settings** → **Service accounts** → _Firebase Admin SDK_ → **Generate new private key** → confirm.
...
**If the key ever leaks:** Google Cloud Console → _IAM & Admin_ → _Service accounts_ → the `firebase-adminsdk` account → _Keys_ → delete the key, then repeat steps 2–3.
...
1. **Authorized domains:** Firebase console → _Authentication_ → _Settings_ → _Authorized domains_. Keep only:
   - `<projectId>.web.app`
   - `<projectId>.firebaseapp.com`
   - any custom domain you add later
   - `localhost` can be removed (local development uses the emulators, not this project).
2. **API key website restriction:** https://console.cloud.google.com → select your project → _APIs & Services_ → _Credentials_ → the key named **"Browser key (auto created by Firebase)"** → _Application restrictions_ → **Websites** → add:
   - `https://<projectId>.web.app/*`
   - `https://<projectId>.firebaseapp.com/*`
     → Save. Leave _API restrictions_ as they are (changing them can break sign-in).
...
3. Settings → _Code security_ → enable **Secret scanning** and **Push protection**.
4. Give the agent the repository URL; the agent adds the remote and pushes **after asking you**.
5. Check the _Actions_ tab: the CI workflow should be green.
...
- In GitHub → Settings → _Secrets and variables_ → _Actions_, add your `VITE_*` values from `web/.env.production.local` as variables/secrets.
