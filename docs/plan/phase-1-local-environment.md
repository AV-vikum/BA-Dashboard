...
4. `tools/src/core/firebase.ts`:
   ...
   - `assertEmulatorRunning()` (emulator target only): `fetch('http://127.0.0.1:8080/')` with a 2s timeout; on failure throw *"Firestore emulator is not running. Start it with: npm run emulators"*.
...
6. Where config lives (link to architecture A4) and the rule: *no real Firebase project is needed until Phase 9.*
