...
| 2026-09-23 | shadcn/ui installed via `-t vite -b radix -p nova` (Radix primitives, Nova/Lucide preset), Geist font pull skipped                    | shadcn CLI changed since the plan was written (preset-based, no "base colour" flag); Nova keeps `lucide-react` and `baseColor: neutral` as originally intended — see note under step 3.1                                                                                            |
| 2026-09-24 | Local CSP/header check uses `vite preview` (reading `firebase.json`'s headers at config-load time), not the Firebase Hosting emulator | The Hosting emulator doesn't apply `firebase.json`'s `headers`/`redirects` (only `rewrites` and static files) — verified directly; the production deploy in Phase 9 still uses `firebase.json` unchanged, and its headers are re-checked live post-deploy — see note under step 3.9 |
